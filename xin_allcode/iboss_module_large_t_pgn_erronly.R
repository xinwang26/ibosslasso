...
ifelse((exists("n") & !is.na(n)),print(n),n<-5e3)
ifelse((exists("p") & !is.na(p)),print(p),p<-500)
...
library(foreach)
library(doParallel)
registerDoParallel(5)  #parallel compute is faster but affect calculate of time cost
...
for(i in 1:rounds)
{
  ...
  lasso_cor02 = reg_module("all",r=floor(0.2*p)/p)
