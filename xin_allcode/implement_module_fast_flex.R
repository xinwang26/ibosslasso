...
#Output:
#######################################################################
 #the default setting for cv.glmnet
...
reg_module<- function(#data will be passed from global environment
  ...
  if(! method %in% c("cor","all","mod","full","rand","lev",'alev','clev','calev',"inno")) {
    print("error!")
    return(NA)} 
  ...
  if(method == "rand"){
    ...
    lasso_time<- system.time(md_cv <- cv.glmnet(rand_x,rand_y,parallel = para_set,alpha=islasso
                                                #,lambda.min.ratio = lam_min_rate
                                                ,family = dist_resp))
  ...
  if(method %in% c("cor","all","mod","lev","inno",'alev','clev','calev')){
    ...
    set.seed(seed_l)
    lasso_time<- system.time(md_cv <- cv.glmnet(sub_x,sub_y,parallel = para_set,alpha=islasso
                                                #,lambda.min.ratio = lam_min_rate
                                                ,family = dist_resp))
    coef_md_cv = coef(md_cv)
    mse.min <-md_cv$cvm[md_cv$lambda == md_cv$lambda.min]
  ...
  if(method == "full"){
    select_time = c(0,0,0)
    lasso_time<- system.time(md_cv <- cv.glmnet(x_mat,y,parallel = para_set,alpha=islasso
                                                #,lambda.min.ratio = lam_min_rate
                                                ,family = dist_resp))
    coef_md_cv = coef(md_cv)
    mse.min <-md_cv$cvm[md_cv$lambda == md_cv$lambda.min]
