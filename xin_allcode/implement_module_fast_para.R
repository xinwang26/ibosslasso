...
#Output:
#######################################################################
split_n_conquer <- function(num_slice=10,vote_rate=0.4,dist_resp = "gaussian", para_set = T,...){
  method = "spc"
...
reg_module<- function(#data will be passed from global environment
  ...
  if(! method %in% c("cor","all","mod","full","rand","lev",'alev','clev','calev',"inno","spc")) {
    print("error!")
    return(NA)} 
  ...
  if(method == "rand"){
    ...
    lasso_time<- system.time(md_cv <- cv.glmnet(rand_x,rand_y,parallel = T,alpha=islasso,family = dist_resp,lambda.min.ratio = 0.0005))
  ...
  if(method == "full"){
    select_time = c(0,0,0)
    lasso_time<- system.time(md_cv <- cv.glmnet(x_mat,y,parallel = T,alpha=islasso,family = dist_resp,lambda.min.ratio = 0.0005))
    coef_md_cv = coef(md_cv)
    mse.min <-md_cv$cvm[md_cv$lambda == md_cv$lambda.min]
    n_selected = n
  }
  if(method == "spc"){
    return(split_n_conquer(num_slice=num_slice,vote_rate=r,dist_resp = "gaussian", para_set = F))
  }
...
#Dependencies: split and conquer with SIS
#Input: 
#Output:
#######################################################################
split_n_conquer_sis <- function(num_slice=10,vote_rate=0.4,sis_rate=0.05,dist_resp = "gaussian", para_set = T,...){
  #######################################################################
  #sort corr
  # cat("slice",num_slice,"vote",vote_rate,"sis",sis_rate)
  (to_select_size = floor(dim(x)[2]*sis_rate))
  ...
  ) %dopar% 
  {
    ...
    slice_end = (i-1)*nk+n_k
    #fit lasso
    slice_x <- sub_x[slice_start:slice_end,]
    slice_y <-  y[slice_start:slice_end]
    md_cv <- cv.glmnet(slice_x,slice_y,parallel = para_set,alpha=islasso,lambda.min.ratio = lam_min_rate,family = dist_resp)
    cvm_rch_min = (md_cv$lambda.min!=md_cv$lambda[length(md_cv$lambda)])
    mse.min <-md_cv$cvm[md_cv$lambda == md_cv$lambda.min]
  ...
  system.time(beta_combine <- E %*% ginv(as.matrix(Reduce(`+`, component))) %*% Reduce(`+`, betai))
  comb_time = proc.time()-tic
  beta_restore = as(matrix(rep(0,to_select_size+1),ncol=1),"dgCMatrix")
  beta_restore[indicates]=beta_combine
  
  temp_coef = beta_restore
...
sis <- function(#data will be passed from global environment
  method,#approach including "all" "cor" "mod" "full"
  cor_r=0.05, #ratio of selected for SIS, vote ratio if split and conquer
  dist_resp = "gaussian", #applicable for logistic regression
  seed_l=seed6,#seed for leverage sampling
  num_slice = 5,
  sis_r = 0.05,
...
{
  if(! method %in% c("cor",'clev','calev','full','rand','spc')) {  #subdata approaches
    print("error!")
    return(NA)} 
  ...
  if(method == "rand"){
    (to_select_size = floor(dim(x)[2]*cor_r))
    corcalc_time <- system.time(cor_resp <- unlist(lapply(x[,-dim(x)[2],with=F],cor,y=y)))
    corsort_time<- system.time(to_select <- sort(abs(cor_resp),decreasing = T,index.return = T)$ix[1:to_select_size])
    select_time = 0
    slct_vars = to_select
  ...
  if(method %in% c("cor","all","mod","lev","inno",'alev','clev','calev')){
    rows_n_cols <- row_select(x,y,subsize,prop = cor_r,seed=seed_l)
  ...
  if(method == "spc"){
    return(split_n_conquer_sis(num_slice=num_slice,vote_rate=cor_r,sis_rate=sis_r,dist_resp = "gaussian", para_set = F))
  }
  return(list(method=method,proprotion=cor_r,coef = coef_md_cv,mse=mse.min,slct_time = select_time,lasso_time =lasso_time[3],n_slct=n_selected,cvm_rch_min = (md_cv$lambda.min!=md_cv$lambda[length(md_cv$lambda)])))
}
