...
raw = raw[sample(1:nrow(raw),nrow(raw)),]
norm_data = cbind(scale(raw[,1:280]),raw[,281]) #normalize
...
for(i in 1:rounds)
{
  ...
  train_index = sample(1:nrow(norm_data),50000)
  test_index = as.numeric(data.frame(data.table(1:nrow(norm_data))[-c(train_index)])[,1])
  train_set = data.table(norm_data[train_index,])
  test_set = data.table(norm_data[test_index,])
  
  x_test = test_set[,c(1:280)]
  x_test = as.matrix(x_test)
  y_test = as.matrix(test_set[,281])
  ...
  time_dopt = system.time(dopt_subdata <- all_vars_max_real(x=xtrain,y = ytrain,sample_size=5600))
  time_cor = system.time(cor02_subdata <- cor_var_max_real(x=xtrain,y = ytrain,sample_size=5600,prop = 0.2))
