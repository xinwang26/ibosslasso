...
para_set = F
source("implement_module_fast_flex.R",echo = F)
#library(irlba)
...
for(i in 1:rounds)
{
  ...
  lasso_cor250 = reg_module("cor",r=250/p)
  convg = convg*(lasso_cor250$cvm_rch_min)
...
#  convg = convg*(lasso_lev$cvm_rch_min)
  ...
  row_cor250 = eval_module(lasso_cor250)
 # row_lev = eval_module(lasso_lev)
  ...
  repo_cor250 = rbind(repo_cor250,row_cor250)
