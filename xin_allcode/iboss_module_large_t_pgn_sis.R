...
#to store result
# repo_all = data.frame()
#repo_corsis100 = data.frame()
#repo_corsis01 = data.frame()
repo_corsis005 = data.frame()
repo_cor005 = data.frame()
repo_rand005 = data.frame()
repo_sislev005  = data.frame()
#repo_sisalev005  = data.frame()
...
for(i in 1:rounds)
{
  ...
  convg = 1
  #IBOSS subset and its lasso regression
  #lasso_corsis100 = reg_module("corsis",r=100/p)
  #convg = convg*(lasso_corsis100$cvm_rch_min)
  
  #lasso_corsis01 = reg_module("corsis",r=0.1)
  #convg = convg*(lasso_corsis01$cvm_rch_min)
  
  lasso_corsis005 = sis("cor",r=0.05)
  ...
  lasso_sislev005  = sis("clev",r=0.05)
  convg = convg*(lasso_sislev005 $cvm_rch_min)
  #lasso_sisalev005  = sis("calev",r=0.05)
  #convg = convg*(lasso_sisalev005 $cvm_rch_min)
  lasso_sisfull = sis("full",r=0.05)
  convg = convg*(lasso_sisfull$cvm_rch_min)
  
  lasso_random = sis("rand",r=0.05)
  convg = convg*(lasso_random$cvm_rch_min)
  
  i=i+1
  if(convg==1){
    # row_all = eval_module(lasso_all)
    #row_corsis100 = eval_module(lasso_corsis100)
    #row_corsis01 = eval_module(lasso_corsis01)
    row_corsis005 = eval_module(lasso_corsis005)
    row_sislev005  = eval_module(lasso_sislev005 )
    #row_sisalev005  = eval_module(lasso_sisalev005 )
    row_cor005 = eval_module(lasso_cor005)
    row_sisfull = eval_module(lasso_sisfull)
    row_rand005 = eval_module(lasso_random)
    
    # repo_all = rbind(repo_all,row_all)
    #repo_corsis100 = rbind(repo_corsis100,row_corsis100)
    #repo_corsis01 = rbind(repo_corsis01,row_corsis01)
    repo_cor005 = rbind(repo_cor005,row_cor005)
    repo_corsis005 = rbind(repo_corsis005,row_corsis005)
    repo_sislev005  = rbind(repo_sislev005 ,row_sislev005 )
    #repo_sisalev005  = rbind(repo_sisalev005 ,row_sisalev005 )
    repo_sisfull = rbind(repo_sisfull,row_sisfull)
    repo_rand005 = rbind(repo_rand005,row_rand005)
  }
  ...
  if(convg==0){
    ...
    rowi = c(i,
             ...
             lasso_sislev005 $cvm_rch_min,            
           #  lasso_sisalev005 $cvm_rch_min,
             lasso_cor005$cvm_rch_min,
             #         lasso_corsis100$cvm_rch_min,
             lasso_corsis005$cvm_rch_min,
             #        lasso_corsis01$cvm_rch_min,
...
saveRDS(rd_record
        ,file=paste0(substr(datetime,1,5),"_n_",n,"_p_",p,"_k_",subsize,"_d_",X_distribution,"_recorsisd.rds"))
